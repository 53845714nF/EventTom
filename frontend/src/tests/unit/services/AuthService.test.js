...
import {
  createCorrectUserSignUp,
  createEmptyUser,
  createCorrectUserSignIn,
  createUserWithPasswordsDontMatch,
  createUserWithPasswordTooShort,
...
describe("AuthService validating User Input", () => {
  test("Expect Correct AuthForm text to be returned", async () => {
    ...
    expect(AuthService.provideDynamicAuthText(false)).toBe(
      AuthFormText.SIGN_IN,
    );
  });
...
describe("AuthService handling user data", () => {
  ...
  test("Expect success toast if axios.post is successfull", async () => {
    ...
    const spyOnCreateToasterPopUp = vi.spyOn(
      ToasterService,
      "createToasterPopUp",
    );
  ...
  test("Expect error toast if testAccessToken is successfull", async () => {
    ...
    expect(spyOnCreateToasterPopUp).toBeCalledWith(
      "success",
      `Token valid. Role: ${testStore.role}`,
    );
    expect(spyOnCreateToasterPopUp).toBeCalledTimes(1);
  });
