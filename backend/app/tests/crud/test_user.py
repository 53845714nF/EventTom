...
def test_create_employee(db: Session) -> None:
    email = random_email()
    password = random_lower_string()
    user_in = UserCreate(email=email, password=password, role=Role.EVENTMANAGER)
    user = crud.create_user(session=db, user_create=user_in)
    assert user.email == email
    assert hasattr(user, "hashed_password")
...
def test_authenticate_employee(db: Session) -> None:
    email = random_email()
    password = random_lower_string()
    user_in = UserCreate(email=email, password=password, role=Role.EVENTCREATOR)
    user = crud.create_user(session=db, user_create=user_in)
    authenticated_user = crud.authenticate(session=db, email=email, password=password)
    assert authenticated_user
    assert user.email == authenticated_user.email
...
def test_check_if_employee_is_not_admin(db: Session) -> None:
    ...
    password = random_lower_string()
    user_in = UserCreate(email=username, password=password, role=Role.EVENTMANAGER)
    user = crud.create_user(session=db, user_create=user_in)
